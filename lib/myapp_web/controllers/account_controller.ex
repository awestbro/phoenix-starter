defmodule MyAppWeb.AccountController do
  ...
  def resend_activation(conn, %{"id" => id}) do
    ...
    cond do
      ...
      user.activation_attempts > Application.get_env(:myapp, :email_activation_max) ->
        ...
        |> put_flash(
          :error,
          "Activation email has been sent too many times. Please contact support to resolve this issue."
        )
  ...
  def send_password_reset_email(conn, %{"email_params" => %{"email" => email}}) do
    ...
    cond do
      user && can_reset_password(user.last_password_reset_attempt) ->
        user =
          Accounts.update_user!(user, %{
            ...
            last_password_reset_attempt: NaiveDateTime.utc_now()
          })
      ...
      user ->
        ...
        |> put_flash(
          :error,
          "Password reset email was recently sent. Please check your email or contact support if you are experiencing issues"
        )
  ...
  def show_password_change(conn, %{"id" => id, "reset_token" => reset_token}) do
    ...
    if reset_token == user.password_reset_token do
      ...
      |> put_flash(
        :error,
        "The password reset token you provided does not match our records. Please contact support if you feel like this is an error"
      )
      |> redirect(to: page_path(conn, :index))
    end
  end
